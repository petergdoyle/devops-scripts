...
if [ $? -eq 127 ]; then
  ...
  make
  if [ $? -ne 0 ]; then
    display_error "\e[7;107;91mcurl-loader - make did not run successfully. skipping.\e[0m"
  else
  ...
  echo -e "\e[7;40;92mcurl-loader already appears to be installed. skipping.\e[0m"
fi
